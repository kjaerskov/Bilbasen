...
For this specific example, we query bilbasen.dk for all VW cars from 2010 to 2016 but limit the search to cars where tax is paid `&includewithoutvehicleregistrationtax=false`, mileage is less than 100.000 km `&priceto=100000` and avoid leased cars `&includeleasing=false`. The content of the webpage is stored in the list `website` using the `read_html` function from `rvest`.
...
And that is it. We now scraped a website using the HTML query string and looping through all the search result pages. Helped by `rvest` and regular expressions. In another post I will use this scraper to fetch data and analyze it. 
